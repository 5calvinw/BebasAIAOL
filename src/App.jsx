...
const App = () => {
  ...
  const handleTypeClick = (plasticTypeID) => {
    setSelectedInfoId(plasticTypeID); 
    setActivePage('information'); 
  };
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {isMobile && (
          <header className="bg-white border-b border-slate-200 p-4 flex items-center justify-between z-10">
            <div className="flex items-center gap-2 font-bold text-lg">
              <div className="w-6 h-6 bg-emerald-500 rounded flex items-center justify-center text-slate-900 text-xs">
                B
              </div>
              BEBAS
            </div>
        ...
        <div className="flex-1 overflow-y-auto h-full">
          {activePage === 'dashboard' && <Dashboard history={history} onViewAll={() => setActivePage('history')} />}
          {activePage === 'scanner' && <Scanner onScanComplete={handleScanComplete} />}

          {/* Pass the click handler to HistoryLog */}
          {activePage === 'history' && <HistoryLog history={history} onTypeClick={handleTypeClick} />}

          {/* Pass the selected ID to InformationPanel */}
          {activePage === 'information' && <InformationPanel initialTypeId={selectedInfoId} />}
